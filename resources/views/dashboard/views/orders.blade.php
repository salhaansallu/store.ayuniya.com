                ...
                <tbody>
                    @foreach (orderTotal('pending', true) as $orders)
                        <tr>
                            ...
                            <td><button onclick="deleteOrder('{{ $orders->id }}')"><i
                                        class="fa-solid fa-trash"></i></button></td>
                        </tr>
                    @endforeach

                    @foreach (orderTotal('processing', true) as $processorders)
                        <tr>
                            <td onclick="getDetails('{{ $processorders->order_number }}')">#{{ $processorders->order_number }}</td>
                            <td>{{ $processorders->created_at }}</td>
                            <td>{{ getOrderTotal($processorders->orders) }}</td>
                            <td>{{ currency($processorders->total_order) }}</td>
                            <td>{{ orderStatus($processorders) }}</td>
                            <td><button onclick="deleteOrder('{{ $processorders->id }}')"><i
                                        class="fa-solid fa-trash"></i></button></td>
                        </tr>
                    @endforeach
                    ...
                    @foreach (orderTotal('delivered', true) as $delorders)
                        <tr>
                            <td onclick="getDetails('{{ $delorders->order_number }}')">#{{ $delorders->order_number }}
                            </td>
                            ...
                            <td><button onclick="deleteOrder('{{ $delorders->id }}')"><i
                                        class="fa-solid fa-trash"></i></button></td>
                        </tr>
                    @endforeach
                    ...
                    @foreach (orderTotal('canceled', true) as $cancelorders)
                        <tr>
                            <td onclick="getDetails('{{ $cancelorders->order_number }}')">
                                #{{ $cancelorders->order_number }}</td>
                            <td>{{ $cancelorders->created_at }}</td>
                            <td>{{ getOrderTotal($cancelorders->orders) }}</td>
                            <td>0.00</td>
                            <td>{{ orderStatus($cancelorders) }}</td>
                            <td><button onclick="deleteOrder('{{ $cancelorders->id }}')"><i
                                        class="fa-solid fa-trash"></i></button></td>
                        </tr>
                    @endforeach
                </tbody>
