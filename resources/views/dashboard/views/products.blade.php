      ...
      <div class="modal-content">
        <div class="modal-body">
          <form action="" method="post" onsubmit="return false;" id="UpdateForm" enctype="multipart/form-data">
            ...
            <div class="txt_field">
              <div class="label">Select vendor</div>
              <div class="input">
                  <select name="vendor" id="vendor" required>
                    <option value="{{ getvendors($pro->vendor)[0]['id'] }}">{{ getvendors($pro->vendor)[0]['company_name'] }} ({{ getvendors($pro->vendor)[0]['store_name'] }})</option>
                    @foreach (getvendors() as $vendor)
                      <option value="{{ $vendor->id }}">{{ $vendor->company_name }} ({{ $vendor->store_name }})</option>
                    @endforeach
                  </select>
              </div>
            </div>
